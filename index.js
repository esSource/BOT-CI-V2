//DISCORD DEVELOPED BY vCesar#0001 in colaboration with hlearn.org
...
for (const folder of commandFolders) {
	const commandFiles = fs.readdirSync(`./addons/${folder}`).filter(file => file.endsWith('.js'));
	console.log(colors.red('Debug: Activando addon de ')+folder)
	for (const file of commandFiles) {
		const command = require(`./addons/${folder}/${file}`);
		client.commands.set(command.name, command);
    console.log(colors.red('Debug: Registrando comando de '+folder+' ')+colors.yellow('$'+command.name))
	}
}
